 ...
 */
void jacobi_step(int nloc, int M, double *x, double *b, double *t)
{
  int rank, prev, next, size, i, j, ld = M + 2;
  ...
  MPI_Send(x + ld * nloc, ld, MPI_DOUBLE, next, 0, MPI_COMM_WORLD);
  MPI_Recv(x + ld * (nloc + 1), ld, MPI_DOUBLE, next, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
{
  int i, j, N = 60, M = 60, ld, size, rank;
